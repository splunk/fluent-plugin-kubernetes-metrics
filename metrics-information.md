...
| kube.sys-container.memory.available_bytes	| float	| bytes	| The storage space available, in bytes, for the filesystem on sys-container.	|
| kube.sys-container.memory.usage_bytes	|float	| bytes	| The node memory, in bytes, in use in the sys-container.	|
| kube.sys-container.memory.rss_bytes	| float	| bytes	| The amount of anonymous and swap cache memory, in bytes, in the sys-container. |	
| kube.sys-container.memory.rss_bytes	| float	| bytes	| The amount of anonymous and swap cache memory, in bytes, in the sys-container.	|
| kube.sys-container.memory.page_faults | integer	|   | The cumulative number of minor page faults in the sys-container.	|
| kube.sys-container.memory.major_page_faults	| integer	|  | The cumulative number of major page faults in the sys-container. |
