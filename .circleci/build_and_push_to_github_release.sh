...
VERSION=`cat VERSION`
echo "Pushing SCK release to github releases..."
ghr -t ${GITHUB_TOKEN} -u ${CIRCLE_PROJECT_USERNAME} -r ${CIRCLE_PROJECT_REPONAME} -c ${CIRCLE_SHA1} -n ${RELEASE_TITLE} -b ${RELEASE_BODY} -draft ${VERSION} ./pkg/